...
def scale_sub(train_x_iter):
    ...
    for i in range(train_x_iter.shape[0]):
        if(np.sum(~np.isnan(train_x_iter[i,:]))==0):
            train_mean[i]=float("nan")
            train_std[i]=float("nan")
            continue
...
# subsample
train_indx = np.genfromtxt(train_size, delimiter=',',dtype=int)

train_x = train_x[:,train_indx]
train_y = train_y[:,train_indx]
train_x_mat = [train_x_mat[i][:,train_indx] for i in range(len(train_x_mat))]
